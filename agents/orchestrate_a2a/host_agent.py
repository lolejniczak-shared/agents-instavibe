...
class HostAgent:
    ...
    def __init__(
        ...
        self.remote_agent_connections: dict[str, 'RemoteAgentConnections'] = {}  ######## Today with no AgentRegistries it is configuration
    ...
    async def create(
        ...
        """
        Asynchronous factory to create and fully initialize a HostAgent.
        """
        http_client = httpx.AsyncClient()
        instance = cls(http_client, task_callback)

        # Now, perform all the async operations on the new instance
        for address in remote_agent_addresses:
            card_resolver = A2ACardResolver(instance.http_client, address)
            card = await card_resolver.get_agent_card()
            remote_connection = RemoteAgentConnections(instance.http_client, card)
            instance.remote_agent_connections[card.name] = remote_connection
            instance.cards[card.name] = card

        # This part depends on the results of the async calls, so it also belongs here
        agent_info = []
